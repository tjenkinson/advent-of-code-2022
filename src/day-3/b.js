...
const getPriority = (letter) => {
  return letter >= 'a'
    ? letter.charCodeAt() - 'a'.charCodeAt() + 1
    : letter.charCodeAt() - 'A'.charCodeAt() + 27;
};

const getDuplicate = (group) => {
  return [...group[0]].find((item) => {
    return group.slice(1).every((groupEntry) => groupEntry.has(item));
  });
};

let total = 0;
for (const group of groups) {
  total += getPriority(getDuplicate(group));
}
